...
export function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/make-server-976c2dfb/send-contact', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setErrorMessage(data.error || 'A apărut o eroare. Vă rugăm să încercați din nou.');
      }
    } catch (error) {
      setSubmitStatus('error');
      setErrorMessage('Nu s-a putut trimite mesajul. Verificați conexiunea la internet.');
    } finally {
